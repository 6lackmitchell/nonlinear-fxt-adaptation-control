...
xs = se.symbols(["x", "y", "vx", "vy"])

# Define symbolic system dynamics
f_symbolic = se.DenseMatrix([xs[2], xs[3], 0, 0])
g_symbolic = se.DenseMatrix([[0, 0], [0, 0], [1, 0], [0, 1]])

# Need to be fixed
s_symbolic_deterministic = se.Matrix([[0 for i in range(2)] for j in range(2)])
s_symbolic_stochastic = se.Matrix([[0 for i in range(2)] for j in range(2)])
